class ShippingWeightsController < ApplicationController
  ...
  def shipping_cal
    ...
    if params[:rate][:items].map(&:vendor).uniq.first == 'Weber Apparel' && params[:rate][:items].map(&:vendor).uniq.count == 1
      if params[:rate][:items].sum(&:quantity) < 50
        shipping_price = WeberShippingRate.where(:min_qty >= params[:rate][:items].sum(&:quantity) && :max_qty <= params[:rate][:items].sum(&:quantity)).first.rate.to_f
      else
        shipping_price = 0.0
      end
    else
