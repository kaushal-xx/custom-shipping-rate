class ShippingWeight < ApplicationRecord
  ...
  def self.get_price(params, ups_rate = true)
    puts "----------------------"
    puts params
    puts "----------------------"
    available_prices = []
    ...
    if weight > 0.0
      if weight < 150
        if weight > light_weight_limit && ups_rate
          ...
          shipping_obj = get_light_weight_shipping_rate(weight, destination_address['country'], destination_address['state'])
          ...
          shipping_obj = ShippingWeight.get_shipping_rate(weight, destination_address['country'], destination_address['state'])
          if shipping_obj.present?
            available_prices << shipping_obj.price.to_f
  ...
  def self.get_ups_rates(params)
    origin_address = params['origin']
    destination_address = params['destination']
    total_weight = params['items'].map{|s| s['grams'] * s['quantity']}.sum
    weight = ('%.2f' % (total_weight*0.0022)).to_f
    origin_details = {country: origin_address[:country], province: origin_address[:state], city: origin_address[:city], zip: origin_address[:zip]}
    destination_details = {country: destination_address['country'], province: destination_address['state'], city: destination_address['city'], zip: destination_address['zip']}
  ...
  def self.get_price_for_api(from_address, to_address, total_weight, ups_rate = false)
    ...
    if weight > 149.00 && weight < 150.00
      ...
      if weight > 0.0
      if weight < 150
            shipping_obj = ShippingWeight.get_light_weight_shipping_rate(weight, destination_address['country'], destination_address['state'])
            if shipping_obj.present?
              available_prices << shipping_obj.price.to_f
              weight_type = 'Light Weight'
            end
            if ups_rate && available_prices.blank?
          origin_details = {country: origin_address['country'], province: origin_address['state'], city: origin_address['city'], zip: origin_address['postal_code']}
          destination_details = {country: destination_address['country'], province: destination_address['state'], city: destination_address['city'], zip: destination_address['postal_code']}
          ...
          weight_type = 'Standard Ground'
            ...
            shipping_obj = get_shipping_rate(weight, destination_address['country'], destination_address['state'])
            if shipping_obj.present?
              available_prices << shipping_obj.price.to_f
