class ShippingWeight < ApplicationRecord
  ...
  def self.ups_shipping_label(params)
      ...
      weight = ('%.2f' % (total_weight*0.0022)).to_f
      destination_address = params['rate']['destination']
      if light_weight_limit(destination_address['country']) > weight
          'Light Weight Expedited'
      elsif light_weight_limit(destination_address['country']) <= weight && weight < 150
  ...
  def self.light_weight_limit(country_iso = nil, max_limit = 150)
      if country_iso.present?
          weight_obj = ShippingWeight.where("weight < ? and country = ? and price != 0", max_limit, country_iso).order("weight").last
      else
          weight_obj = ShippingWeight.where("weight < ? and price != 0", max_limit).order("weight").last
      end
